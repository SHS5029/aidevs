...
if st.button("과정 목록 불러오기"):  # 버튼을 누른 순간에만 과정 목록 API를 호출합니다.
    ...
    count = data.get("count", 0)  # count key가 없을 때를 대비해 기본값으로 0을 사용합니다.
    st.info(f"총 {count}개의 과정이 조회되었습니다.")
    ...
    st.dataframe(pd.DataFrame({"과정명": courses}))  # 과정 목록을 데이터프레임 형태로 화면에 출력합니다.