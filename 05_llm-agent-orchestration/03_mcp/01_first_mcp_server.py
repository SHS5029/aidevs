...
@mcp.tool()
def get_hotels(city: Literal["부산", "서울"], spec: str) -> dict:
    """도시와 호텔의 사양(spec)에 맞는 호텔을 조회합니다."""
    normalized = city.strip()
    if not normalized:
        raise ValueError("city는 빈 문자열일 수 없습니다.")
    return {
        "city": normalized,
        "items": [
            {"name": "바다 호텔", "city": "부산", "spec": "바다 전망, 금연, 조식 포함", "price": 120_000},
            {"name": "도시 호텔", "city": "서울", "spec": "도시 전망, 금연, 조식 미포함", "price": 140_000},
            {"name": "강변 호텔", "city": "부산", "spec": "강변 전망, 흡연 가능, 조식 포함", "price": 130_000},
            {"name": "산 전망 호텔", "city": "서울", "spec": "산 전망, 금연, 조식 포함", "price": 150_000},
        ],
        "source": "travel-hotel-spec-catalog",
    }
