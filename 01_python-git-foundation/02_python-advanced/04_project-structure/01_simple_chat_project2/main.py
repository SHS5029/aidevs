...
def main() -> None:
    while True:
        ...
        print("처리중,,,,")
        try:
            request_message = RequestChatMessage(prompt=question, user="테스트유저")
            response_message = create_chat_message(request_message)
            print(f"답변: {response_message.answer}  {response_message.model} {response_message.msg}")
            print()
